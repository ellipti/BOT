...
def render_chart_with_overlays(
    ...
    if d.index.min().year < 2000:
        d.index = pd.to_datetime(
            d.index.astype(int) // 10**9, unit="s", errors="coerce"
        )
        d = d[~d.index.isna()]
        d = d[~d.index.duplicated()]
        d = d.sort_index()
    ...
    ), f"Index is not DatetimeIndex: {type(d.index)}"
    assert (
        d.index.min().year > 2000
    ), f"Min year is {d.index.min().year}, check time data!"
    ...
    logger.info(
        f"plot_df.index dtype: {plot_df.index.dtype}; sample: {plot_df.index[:5].tolist()}"
    )
    ...
    if "candlestick_ohlc" not in globals() or candlestick_ohlc is None:
        ...
        ax = axlist[0]
        ax_vol = (
            axlist[2] if len(axlist) > 2 else (axlist[1] if len(axlist) > 1 else None)
        )
    else:
        # Convert cleaned index to timezone-aware then naive times, then to matplotlib floats
        times = (
            pd.to_datetime(d.index, utc=True)
            .tz_convert("Asia/Ulaanbaatar")
            .tz_localize(None)
        )
        ...
        if "Volume" in d.columns:
            ...
            colors = [
                "white" if c >= o else "#00A7E1"
                for o, c in zip(
                    d["Open"].to_numpy(), d["Close"].to_numpy(), strict=False
                )
            ]
    ...
    for tl in overlays.get("trendlines", []):
        t1 = pd.to_datetime(tl["anchor_a"]["time"])
        p1 = tl["anchor_a"]["price"]
        t2 = pd.to_datetime(tl["anchor_b"]["time"])
        p2 = tl["anchor_b"]["price"]
    ...
    for fb in overlays.get("fibonacci", []):
        hi = fb["swing_high"]["price"]
        lo = fb["swing_low"]["price"]
    ...
    def _annotate_levels(ax, entry=None, sl=None, tp=None):
        try:
            ...
            x_pos = None
        items = ((entry, "Entry", "yellow"), (sl, "SL", "red"), (tp, "TP", "lime"))
