...
from services.telegram_v2 import TelegramBotV2, send_photo_sync, send_text_sync

logger = get_logger("telegram_notify")
settings = get_settings()

# Global bot instance
_bot_instance: TelegramBotV2 | None = None


def get_bot() -> TelegramBotV2 | None:
    ...
    if _bot_instance is None and settings.telegram.bot_token:
        ...
        _bot_instance = TelegramBotV2(settings.telegram.bot_token, chat_ids)
...
def send_text(text: str, parse_mode: str | None = None) -> None:
    ...
    if success:
        logger.info(
            f"Telegram message sent: {text[:50]}{'...' if len(text) > 50 else ''}"
        )
    else:
...
def send_error_alert(error_msg: str, context: str | None = None) -> None:
...
def send_bot_status(status: str, details: str | None = None) -> None:
...
async def send_text_async(text: str, parse_mode: str | None = None) -> bool:
...
async def send_photo_async(path: str | Path, caption: str | None = None) -> bool:
