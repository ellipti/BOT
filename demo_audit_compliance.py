...
from audit.audit_logger import get_audit_logger, audit_event, audit_order, audit_fill, audit_config, audit_auth
...
def demo_audit_logging():
    ...
    audit_config("risk_percentage", old_value=0.02, new_value=0.015, file_path="configs/risk.yaml")
    ...
    # Alert event
    audit_event("AlertSent", alert_type="price_target", symbol="XAUUSD", 
                message="Gold price reached target level", severity="HIGH")
    print("  ✅ Alert event logged")
    
    # Stop loss update
    audit_event("StopUpdated", symbol="GBPUSD", old_sl=1.2500, new_sl=1.2520, 
                reason="trailing_stop", order_id="ORD002")
    ...
    if today_log.exists():
        ...
        with open(today_log, 'r') as f:
            ...
            for line in lines[-3:]:  # Show last 3 events
                ...
                timestamp = datetime.fromisoformat(record['iso_ts'].replace('Z', '+00:00'))
                print(f"   {timestamp.strftime('%H:%M:%S')} - {record['event']} "
...
def demo_config_snapshots():
    ...
    for file_path, file_info in list(snapshot['files'].items())[:5]:  # Show first 5
        print(f"   {Path(file_path).name}: {file_info['size']} bytes, "
...
def demo_redaction_system():
    ...
    if today_log.exists():
        with open(today_log, 'r') as f:
            ...
            print(f"✅ Raw log entry (with redaction applied):")
            print(f"   {last_line.strip()}")
            
            # Verify redaction worked
            if "[REDACTED]" in last_line:
                print(f"✅ Sensitive data successfully redacted")
            else:
                print(f"⚠️  Note: Redaction patterns may need adjustment")
...
def demo_audit_statistics():
    ...
    if artifacts_dir.exists():
        export_dirs = [d for d in artifacts_dir.iterdir() if d.is_dir() and len(d.name) == 10]
        ...
        for export_dir in sorted(export_dirs)[-3:]:  # Show last 3
            ...
            if manifest_file.exists():
                with open(manifest_file) as f:
                    manifest = json.load(f)
                print(f"   {export_dir.name}: {manifest['summary']['total_files']} files")
