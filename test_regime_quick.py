...
def create_test_candles(volatility: float, count: int = 30) -> list[Candle]:
    ...
    for i in range(count):
        ...
        candle = Candle(
            ts=i * 3600, open=open_price, high=high, low=low, close=close, volume=1000.0
        )
...
def test_regime_detection():
    ...
    for regime in ["low", "normal", "high"]:
        ...
        print(
            f"   {regime.upper()}: RISK_PCT={params['RISK_PCT']:.3f}, "
    ...
    print(
        f"   Thresholds: low={config.thresholds.low}, normal={config.thresholds.normal}, high={config.thresholds.high}"
    )
