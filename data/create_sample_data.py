...
def generate_sample_ohlcv(
    ...
    for i, ts in enumerate(timestamps):
        ...
        high_price = max(open_price, close_price) + np.random.exponential(
            volatility * 0.3
        )
        low_price = min(open_price, close_price) - np.random.exponential(
            volatility * 0.3
        )
...
def create_all_sample_data():
    ...
    for symbol in symbols:
        for tf_minutes in timeframes:
            ...
            print(
                f"  Date range: {pd.to_datetime(df['ts'].min(), unit='s').strftime('%Y-%m-%d')} to {pd.to_datetime(df['ts'].max(), unit='s').strftime('%Y-%m-%d')}"
            )
