...
def place_market(symbol: str, side: str, lot: float, sl: float | None, tp: float | None, retries=2, pause=0.7):
    ...
    if info.trade_mode != 4:
        logger.error(f"Market is not fully tradeable for {symbol} (trade_mode={info.trade_mode})")
    ...
    for i in range(retries+1):
        ...
        logger.warning(f"order_send fail (attempt {i+1}/{retries+1}) retcode={getattr(res,'retcode',None)}")
...
def compute_stops(symbol: str, side: str, entry: float, sl_points: float, tp_points: float) -> Tuple[float, float]:
    ...
    if side == "BUY":
        if entry - sl < stop_dist: sl = round(entry - stop_dist, digits)
        if tp - entry < stop_dist: tp = round(entry + stop_dist, digits)
    else:
        if sl - entry < stop_dist: sl = round(entry + stop_dist, digits)
        if entry - tp < stop_dist: tp = round(entry - stop_dist, digits)
    return sl, tp


def place_with_fallback(symbol: str, side: str, volume: float, entry: float, 
                       sl_price: float, tp_price: float, deviation: int = 30) -> Dict[str, Any]:
    ...
    if r and r.retcode in RETCODE_OK:
        ...
        if pos:
            ticket = pos[0].ticket
            mod = {"action": mt5.TRADE_ACTION_SLTP, "position": ticket, "sl": sl_price, "tp": tp_price}
            mt5.order_send(mod)
