    ...
    def jwt_secret(self) -> str:
        ...
        if self._jwt_secret is None:
            ...
            if not self._jwt_secret:
                ...
                logger.warning(f"No DASH_JWT_SECRET configured, using dev secret: {dev_secret}")
...
def log_audit_event(event_type: str, user_id: str = None, details: dict = None, success: bool = True):
    ...
    try:
        ...
        logger.info(f"Audit: {event_type} - {user_id} - {'SUCCESS' if success else 'FAILED'}")
...
async def optional_user(request: Request) -> Optional[Tuple[str, List[str]]]:
