...
from dashboard.auth import hash_password, verify_password, get_user_roles, log_audit_event
...
class UserStore:
    ...
    def create_user(self, email: str, password: str, roles: List[str] = None) -> str:
        ...
        try:
            with self._get_connection() as conn:
                ...
                log_audit_event("user_created", user_id=user_id, details={"email": email, "roles": roles})
    ...
    def authenticate_user(self, email: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            row = cursor.fetchone()
            if not row:
                log_audit_event("login_failed", details={"email": email, "reason": "user_not_found"}, success=False)
                return None
            
            # Verify password
            if not verify_password(password, row["pwd_hash"]):
                log_audit_event("login_failed", user_id=row["id"], details={"email": email, "reason": "invalid_password"}, success=False)
                return None
    ...
    def update_user_roles(self, user_id: str, roles: List[str]) -> bool:
        ...
        with self._get_connection() as conn:
            ...
            if cursor.rowcount > 0:
                conn.commit()
                log_audit_event("user_roles_updated", user_id=user_id, details={"new_roles": roles})
                return True
