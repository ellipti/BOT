...
class PaperBroker(BrokerGateway):
    ...
    def __init__(self, settings, price_feed=None):
        ...
        self._market_data = {
            ...
            "EURUSD": {"bid": 1.0950, "ask": 1.0952, "point": 0.00001, "pip_value": 10.0},
            "GBPUSD": {"bid": 1.2650, "ask": 1.2653, "point": 0.00001, "pip_value": 10.0},
        ...
        if self.price_feed:
            try:
                ...
                if tick:
                    ...
                    slippage = self._slippage_pips * point * 10  # Convert pips to points
                    if side == Side.BUY:
    ...
    def _update_position(self, symbol: str, qty_change: float, avg_price: float):
        ...
        if current_pos is None:
            ...
            if abs(new_qty) < 0.001:  # Position closed (considering floating point precision)
                del self._positions[symbol]
                logger.debug(f"Position closed: {symbol}")
            else:
                # Update average price using weighted average
                if (current_pos.qty > 0 and qty_change > 0) or (current_pos.qty < 0 and qty_change < 0):
                    # Adding to position in same direction
                    total_cost = (current_pos.qty * current_pos.avg_price) + (qty_change * avg_price)
                    new_avg_price = total_cost / new_qty
                else:
                    # Reducing position or reversing
                    new_avg_price = avg_price if abs(qty_change) > abs(current_pos.qty) else current_pos.avg_price
                ...
                logger.debug(f"Updated position: {symbol} qty={new_qty} @ {new_avg_price}")
    ...
    def place_order(self, request: OrderRequest) -> OrderResult:
        ...
        try:
            ...
            if request.client_order_id in self._orders:
                ...
                logger.info(f"Duplicate order ID {request.client_order_id}, returning existing result")
                return OrderResult(
                    accepted=True,
    ...
    def cancel(self, broker_order_id: str) -> bool:
        ...
        """
        logger.warning(f"Paper broker order cancellation requested for {broker_order_id} (not supported)")
        return False
