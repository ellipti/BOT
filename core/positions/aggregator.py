...
class NettingResult:
    ...
    net_position_side: str | None  # Final net position side if any
    summary: str
...
class PositionAggregator:
    ...
    def __init__(
        self,
        netting_mode: NettingMode = NettingMode.NETTING,
        ...
        logger.info(
            f"PositionAggregator initialized: mode={netting_mode}, rule={reduce_rule}"
        )
    ...
    def _process_netting(
        ...
        if volume >= total_opposite_volume:
            ...
            reduce_actions = self._create_partial_reduction(
                opposite_positions, volume, price
            )
            avg_close_price = self._calculate_average_price(
                [
                    p
                    for p in opposite_positions
                    if any(a.position_ticket == p.ticket for a in reduce_actions)
                ]
            )
    ...
    def _create_full_reduction(
        self, positions: list[Position], close_price: float
    ) -> list[ReduceAction]:
        ...
        for position in positions:
            actions.append(
                ReduceAction(
                    ...
                    reason="Full closure via netting",
                )
            )
        return actions

    def _create_partial_reduction(
        self, positions: list[Position], total_volume: float, close_price: float
    ) -> list[ReduceAction]:
    ...
    def _reduce_fifo(
        ...
        return self._reduce_sequential(
            sorted_positions, total_volume, close_price, "FIFO"
        )
    ...
    def _reduce_lifo(
        ...
        return self._reduce_sequential(
            sorted_positions, total_volume, close_price, "LIFO"
        )
