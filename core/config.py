...
class Strategy:
    def __init__(self):
        ...
        self.mt5_path = (
            settings.MT5_PATH or "C:\\Program Files\\MetaTrader 5\\terminal64.exe"
        )
    ...
    def using_attach_mode(self) -> bool:
        ...
        return self.attach_mode or not all(
    ...
    def validate_trade_decision(self, decision: dict) -> tuple[bool, str]:
        ...
        if risk["r_multiple"] < self.min_risk_reward:
            return (
                False,
                f"Risk/Reward {risk['r_multiple']:.2f} below minimum {self.min_risk_reward}",
            )
