...
class ProfileSettings(BaseModel):
    ...
    prefer_profile: bool = Field(
        default=True, description="Prefer profile over MT5 info"
    )
    strict_session: bool = Field(
        default=True, description="Block trades outside sessions"
    )
    check_holidays: bool = Field(default=False, description="Enable holiday checking")
    override_sizing: bool = Field(
        default=True, description="Override sizing with profiles"
    )
...
def load_symbol_profiles(
    config_path: str = "configs/symbol_profiles.yaml",
) -> ProfileConfig:
    """
    ...
    return ProfileConfig(
        ...
        sessions={
            "24x5": SessionDefinition(
                days=[0, 1, 2, 3, 4], start_time="22:00", end_time="22:00", breaks=[]
            ),
            ...
            "RTH": SessionDefinition(
                days=[0, 1, 2, 3, 4], start_time="14:30", end_time="21:00", breaks=[]
            ),
        },
...
class SymbolProfileManager:
    ...
    def __init__(self, config: ProfileConfig | None = None):
        ...
        logger.info(
            f"SymbolProfileManager initialized with {len(self.config.profiles)} profiles"
        )
    ...
    def is_session_open(self, symbol: str, dt: datetime | None = None) -> bool:
        """
        ...
        try:
            ...
            if weekday not in session_def.days:
                logger.debug(
                    f"Session closed: {symbol} not trading on {local_dt.strftime('%A')}"
                )
                return False
            ...
            if is_open and session_def.breaks:
                for break_period in session_def.breaks:
                    ...
                    if break_start <= current_time <= break_end:
                        logger.debug(
                            f"Session closed: {symbol} in break period {break_start}-{break_end}"
                        )
                        return False

            if not is_open:
                logger.debug(
                    f"Session closed: {symbol} outside {start_time}-{end_time} in {profile.tz}"
                )
    ...
    def is_holiday(self, symbol: str, date: datetime | None = None) -> bool:
        """
    ...
    def can_trade(self, symbol: str, dt: datetime | None = None) -> tuple[bool, str]:
        """
    ...
    def get_session_summary(self, symbol: str) -> dict:
        ...
        return {
            ...
            "session_hours": (
                f"{session_def.start_time}-{session_def.end_time}"
                if session_def
                else "Unknown"
            ),
        }
...
_profile_manager: SymbolProfileManager | None = None
