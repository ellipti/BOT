...
class GuardrailEvaluator:
    ...
    def _evaluate_guardrails(self) -> None:
        ...
        for arm, arm_metrics in self.metrics.items():
            ...
            if reject_rate_15m > reject_max:
                violations.append(
                    f"Arm {arm}: rejection rate {reject_rate_15m:.3f} > {reject_max} (15m)"
                )
            if reject_rate_60m > reject_max:
                violations.append(
                    f"Arm {arm}: rejection rate {reject_rate_60m:.3f} > {reject_max} (60m)"
                )
            ...
            if timeout_rate_15m > timeout_max:
                violations.append(
                    f"Arm {arm}: fill timeout rate {timeout_rate_15m:.3f} > {timeout_max} (15m)"
                )
            if timeout_rate_60m > timeout_max:
                violations.append(
                    f"Arm {arm}: fill timeout rate {timeout_rate_60m:.3f} > {timeout_max} (60m)"
                )
            ...
            if drawdown_15m > drawdown_max:
                violations.append(
                    f"Arm {arm}: drawdown {drawdown_15m:.3f} > {drawdown_max} (15m)"
                )
            if drawdown_60m > drawdown_max:
                violations.append(
                    f"Arm {arm}: drawdown {drawdown_60m:.3f} > {drawdown_max} (60m)"
                )
    ...
    def _trigger_rollback(self, violations: list[str]) -> None:
        ...
        logger.error(
            f"GUARDRAIL VIOLATION - Triggering rollback: {self.rollback_reason}"
        )
    ...
    def _apply_rollback_weights(self) -> None:
        ...
        try:
            safe_weights = self.config.get("rollback", {}).get(
    ...
    def get_guardrail_status(self) -> dict:
        ...
        status = {
            ...
            "rollback_time": (
                self.rollback_time.isoformat() if self.rollback_time else None
            ),
        ...
        for arm, arm_metrics in self.metrics.items():
            ...
            arm_status["violations"] = {
                "rejection_rate": max(
                    arm_status["rejection_rate_15m"], arm_status["rejection_rate_60m"]
                )
                > guardrails.get("rejected_rate_max", 0.05),
                "fill_timeout": max(
                    arm_status["fill_timeout_rate_15m"],
                    arm_status["fill_timeout_rate_60m"],
                )
                > guardrails.get("fill_timeout_max", 0.02),
                "drawdown": max(arm_status["drawdown_15m"], arm_status["drawdown_60m"])
                > guardrails.get("drawdown_day_max", 0.03),
            }
