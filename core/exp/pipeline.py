...
class ExperimentPipelineIntegrator:
    ...
    def handle_signal_detected(self, event: SignalDetected) -> SignalDetected:
        ...
        try:
            ...
            logger.debug(
                f"Assigned {event.symbol} signal to experiment arm {arm_name} (strategy: {experiment_strategy_id})"
            )
    ...
    def handle_order_placed(self, event: OrderPlaced) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Recorded order placement for arm {arm_name}: {event.client_order_id}"
            )
    ...
    def handle_order_filled(self, event: Filled) -> None:
        ...
        try:
            ...
            if order_info:
                ...
                fill_latency_ms = (
                    event.ts - order_info["placement_time"]
                ).total_seconds() * 1000

                # Record fill metrics
                self.metrics.inc("experiment_order_filled", symbol=symbol, arm=arm_name)

                self.metrics.observe(
                    "experiment_fill_latency_ms",
                    fill_latency_ms,
                    symbol=symbol,
                    arm=arm_name,
                )
                ...
                logger.debug(
                    f"Recorded fill for arm {arm_name}: {fill_latency_ms:.1f}ms latency"
                )
            else:
                logger.warning(
                    f"No placement info found for filled order: {event.client_order_id}"
                )
    ...
    def handle_order_rejected(self, event: Rejected) -> None:
        ...
        try:
            ...
            if order_info:
                ...
                logger.warning(
                    f"No placement info found for rejected order: {event.client_order_id}"
                )
...
_integrator: ExperimentPipelineIntegrator | None = None
...
if __name__ == "__main__":
    ...
    signal = SignalDetected(
        symbol="XAUUSD", side="BUY", strength=0.85, strategy_id="baseline_ma"
    )
