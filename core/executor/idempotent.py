...
from core.positions import NettingMode, Position, PositionAggregator, ReduceRule
...
class IdempotentOrderExecutor:
    ...
    def __init__(
        ...
        db_path: str = "infra/id_store.sqlite",
        ...
        logger.info(
            f"IdempotentOrderExecutor initialized with db: {self.db_path}, "
    ...
    def _execute_with_netting(self, request: OrderRequest) -> OrderResult:
        ...
        if netting_result.reduce_actions:
            for action in netting_result.reduce_actions:
                ...
                if not reduce_result.accepted:
                    logger.warning(
                        f"Reduce action failed: {action.position_ticket} - {reduce_result.reason}"
                    )
        ...
        if netting_result.remaining_volume > 0:
            ...
            if main_result.accepted:
                ...
                logger.info(
                    f"Order accepted and recorded: {client_order_id} -> {main_result.broker_order_id}"
                )
            else:
                logger.warning(
                    f"Order rejected by broker: {client_order_id} - {main_result.reason}"
                )
        else:
    ...
    def _get_existing_positions(self, symbol: str) -> list[Position]:
        ...
        try:
            ...
            if hasattr(self.broker, "get_positions"):
                ...
                logger.warning(
                    "Broker does not support get_positions, assuming no existing positions"
                )
                return []
    ...
    def _execute_reduce_action(self, action) -> OrderResult:
        ...
        try:
            ...
            reduce_request = OrderRequest(
                ...
                side=(
                    "SELL" if action.position_ticket.startswith("BUY") else "BUY"
                ),  # Opposite side
            ...
            if hasattr(self.broker, "close_position"):
                result = self.broker.close_position(
                    action.position_ticket, action.reduce_volume
                )
            else:
            ...
            logger.info(
                f"Reduce action executed: {action.position_ticket} "
    ...
    def _send_netting_summary(self, netting_result, reduce_results, main_result):
        ...
        try:
            ...
            if reduce_results:
                closed_volume = sum(
                    action.reduce_volume for action in netting_result.reduce_actions
                )
