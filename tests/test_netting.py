...
from core.positions import (
    PositionAggregator, Position, NettingMode, ReduceRule, 
    NettingResult, ReduceAction
)
...
class TestNettingPolicy:
    ...
    def create_position(self, ticket: str, side: str, volume: float, 
                       entry_price: float, minutes_ago: int = 0) -> Position:
    ...
    def test_hedging_mode_no_netting(self):
        ...
        existing_positions = [
            self.create_position("1001", "BUY", 1.0, 2500.0, 10)
        ]
        
        # Incoming short order
        result = aggregator.process_incoming_order(
            symbol="XAUUSD", side="SELL", volume=0.6, price=2510.0,
    ...
    def test_netting_partial_reduction_fifo(self):
        ...
        result = aggregator.process_incoming_order(
            symbol="XAUUSD", side="SELL", volume=0.6, price=2515.0,
            existing_positions=existing_positions
        )
    ...
    def test_netting_proportional_reduction(self):
        ...
        result = aggregator.process_incoming_order(
            symbol="XAUUSD", side="SELL", volume=0.5, price=2515.0,
            existing_positions=existing_positions
        )
    ...
    def test_netting_full_closure_with_remaining(self):
        ...
        result = aggregator.process_incoming_order(
            symbol="XAUUSD", side="SELL", volume=1.2, price=2515.0,
            existing_positions=existing_positions
        )
    ...
    def test_netting_no_opposite_positions(self):
        ...
        result = aggregator.process_incoming_order(
            symbol="XAUUSD", side="BUY", volume=0.3, price=2510.0,
            existing_positions=existing_positions
        )
...
class TestIdempotentExecutorNetting:
    ...
    def test_executor_netting_mode_reduction(self, mock_datetime):
        ...
        self.mock_broker.close_position = MagicMock(return_value=OrderResult(
        ...
        assert (self.mock_broker.close_position.called or 
                self.mock_broker.place_order.called)
