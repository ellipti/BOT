...
class RegimeConfig(BaseModel):
    ...
    min_regime_duration: int = Field(default=3, ge=1, description="Min bars before regime change")
    regime_confidence: float = Field(default=0.85, ge=0.5, le=1.0, description="Confidence threshold")
...
def compute_norm_atr(candles: List[Candle], atr_window: int) -> float:
    ...
    if len(candles) < atr_window + 1:
        logger.warning(f"Insufficient candles for ATR: {len(candles)} < {atr_window + 1}")
        return 0.0
    ...
    try:
        ...
        logger.debug(f"ATR calculation: TR count={len(true_ranges)}, "
...
class RegimeDetector:
    ...
    def __init__(self, config: Optional[RegimeConfig] = None):
        ...
        self._regime_start_time: Optional[int] = None
        
        logger.info(f"RegimeDetector initialized: active={self.cfg.active}, "
    ...
    def detect(self, candles: List[Candle], symbol: str = "UNKNOWN") -> RegimeType:
        ...
        try:
            ...
            logger.info(f"Regime detection [{symbol}]: norm_ATR={norm_atr:.6f}, "
    ...
    def _apply_stability(self, raw_regime: RegimeType) -> RegimeType:
        ...
        if consistency >= self.cfg.regime_confidence:
            ...
            logger.debug(f"Regime stability: keeping {current_regime} "
