...
from .runner import BacktestResult, BacktestParams
...
class LabVisualizer:
    ...
    def plot_best_equity_curve(self, best_result: BacktestResult, config: dict[str, Any]) -> str:
        """
        ...
        if 'trades' in equity_data and equity_data['trades']:
            ...
            winning_equity = [t['equity_after'] for t in equity_data['trades'] if t['pnl'] > 0]
            losing_dates = [t['date'] for t in equity_data['trades'] if t['pnl'] <= 0]  
            losing_equity = [t['equity_after'] for t in equity_data['trades'] if t['pnl'] <= 0]
            
            ax1.scatter(winning_dates, winning_equity, color='green', marker='^', s=50, label='Winning Trades', alpha=0.7)
            ax1.scatter(losing_dates, losing_equity, color='red', marker='v', s=50, label='Losing Trades', alpha=0.7)
        
        ax1.set_title(f'Equity Curve - {best_result.params}', fontsize=14, fontweight='bold')
...
Total P&L: ${best_result.total_pnl:,.2f}"""
    ...
    def plot_parameter_heatmaps(self, results: list[BacktestResult]) -> list[str]:
        ...
        if len(df['risk_pct'].unique()) > 1 and len(df['sl_mult'].unique()) > 1:
            ...
            pivot_data = df.groupby(['risk_pct', 'sl_mult'])['profit_factor'].mean().unstack()
            sns.heatmap(pivot_data, annot=True, fmt='.2f', cmap='RdYlGn', ax=ax, cbar_kws={'label': 'Profit Factor'})
            
            ax.set_title('Profit Factor by Risk % and Stop Loss Multiplier', fontsize=14, fontweight='bold')
        ...
        if len(df['sl_mult'].unique()) > 1 and len(df['tp_mult'].unique()) > 1:
            ...
            sns.heatmap(pivot_data, annot=True, fmt='.1%', cmap='Blues', ax=ax, cbar_kws={'label': 'Win Rate'})
            
            ax.set_title('Win Rate by Stop Loss and Take Profit Multipliers', fontsize=14, fontweight='bold')
        ...
        if len(df['symbol'].unique()) > 1 and len(df['timeframe'].unique()) > 1:
            ...
            pivot_data = df.groupby(['symbol', 'timeframe'])['total_pnl'].mean().unstack()
            sns.heatmap(pivot_data, annot=True, fmt='.0f', cmap='RdYlGn', ax=ax, 
                       cbar_kws={'label': 'Average Total P&L ($)'})
            
            ax.set_title('Average P&L by Symbol and Timeframe', fontsize=14, fontweight='bold')
    ...
    def plot_performance_distribution(self, results: list[BacktestResult]) -> str:
        ...
        profit_factors = [r.profit_factor for r in successful_results if r.profit_factor < 10]  # Remove outliers
        ...
        ax1.axvline(sum(win_rates) / len(win_rates), color='red', linestyle='--', label=f'Mean: {sum(win_rates)/len(win_rates):.1f}%')
        ...
        ax2.hist(profit_factors, bins=20, alpha=0.7, color='lightgreen', edgecolor='black')
        ax2.axvline(sum(profit_factors) / len(profit_factors), color='red', linestyle='--', 
        ...
        ax3.axvline(sum(max_dds) / len(max_dds), color='red', linestyle='--', label=f'Mean: {sum(max_dds)/len(max_dds):.1f}%')
        ...
        ax4.axvline(sum(total_pnls) / len(total_pnls), color='red', linestyle='--', 
    ...
    def _get_detailed_equity_curve(self, params: BacktestParams, config: dict[str, Any]) -> dict[str, Any] | None:
        """
        ...
        """
        try:
            from .runner import run_single_backtest, add_technical_indicators, simulate_backtest
            from config.settings import get_settings
            from feeds.factory import create_feed
            ...
            # Get data
            feed = create_feed("backtest", settings, data_dir=config["data"]["data_dir"])
            timeframe_str = f"M{params.timeframe}"
            candles = feed.get_ohlcv(params.symbol, timeframe_str, config["data"]["min_bars"])
            ...
            for i, row in df.iterrows():
                ...
                if position is None:
                    decision = guard.filter_decision(
                        row['raw_signal'], current_price, row['ma20'], row['ma50'], row['rsi14'], row['atr'], balance
                    )
...
def create_visualizations(results: list[BacktestResult], config: dict[str, Any]) -> dict[str, str]:
    """
    ...
    try:
        ...
        if successful_results:
            ...
            try:
                chart_paths['equity_curve'] = visualizer.plot_best_equity_curve(best_result, config)
            except Exception as e:
                logger.error(f"Failed to create equity curve: {e}")
            
            # Create performance distributions
            try:
                chart_paths['distributions'] = visualizer.plot_performance_distribution(successful_results)
            except Exception as e:
