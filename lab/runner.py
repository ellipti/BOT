...
class StrategyLabRunner:
    ...
    def generate_parameter_grid(self) -> list[BacktestParams]:
        ...
        combinations = list(
            product(symbols, timeframes, atr_periods, risk_pcts, sl_mults, tp_mults)
        )
    ...
    def run_grid_search(self, max_workers: int = 4) -> list[BacktestResult]:
        ...
        logger.info(
            f"Starting grid search with {len(params_list)} combinations using {max_workers} workers"
        )
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_params), 1):
                ...
                try:
                    ...
                    if result.success:
                        logger.info(
                            f"[{i}/{len(params_list)}] OK {params} -> WR: {result.win_rate:.1%}, PF: {result.profit_factor:.2f}"
                        )
                    else:
                        logger.warning(
                            f"[{i}/{len(params_list)}] FAIL {params} -> Error: {result.error}"
                        )

                except Exception as e:
                    logger.error(
                        f"[{i}/{len(params_list)}] ERROR {params} -> Exception: {e}"
                    )
                    results.append(
                        BacktestResult(params=params, success=False, error=str(e))
                    )
        ...
        logger.info(
            f"Grid search completed in {total_time:.1f}s: {successful}/{len(results)} successful"
        )
    ...
    def generate_summary_report(self, results: list[BacktestResult]) -> None:
        ...
        top_results = sorted(
            successful_results, key=lambda r: r.profit_factor, reverse=True
        )[:top_n]
...
def run_single_backtest(
    params: BacktestParams, config: dict[str, Any]
) -> BacktestResult:
    """
    ...
    try:
        ...
        settings.feed.backtest_data_dir = config["data"][
            "data_dir"
        ]  # Set data directory
...
def add_technical_indicators(df: pd.DataFrame, atr_period: int) -> pd.DataFrame:
    ...
    df.loc[
        (df["ma20"] > df["ma50"]) & (df["ma20"].shift() <= df["ma50"].shift()),
        "raw_signal",
    ] = "BUY"
    df.loc[
        (df["ma20"] < df["ma50"]) & (df["ma20"].shift() >= df["ma50"].shift()),
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("lab/out/run.log")],
    )

    # Command line interface
    parser = argparse.ArgumentParser(
        description="Strategy Lab Parameter Grid Backtesting"
    )
    parser.add_argument(
        "--config", default="lab/params.yaml", help="Configuration file path"
    )
