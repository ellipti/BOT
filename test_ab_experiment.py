...
from core.events.types import Filled, OrderPlaced, Rejected, SignalDetected
...
def test_guardrail_system():
    ...
    for arm, arm_status in status.get("arms", {}).items():
        ...
        print(
            f"   Arm {arm}: {rejection_rate:.1%} rejection rate ({sample_count} samples)"
        )
...
def test_experiment_metrics():
    ...
    print(
        f"   Assignment stats: {metrics.get('assignment', {}).get('total_assignments', 0)} assignments"
    )
    print(f"   Active orders: {metrics.get('active_orders', 0)}")
    print(
        f"   Guardrail status: {'active' if metrics.get('guardrails', {}).get('rollback_active') else 'normal'}"
    )
