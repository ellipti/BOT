...
class DRDrillRunner:
    ...
    def __init__(self, dry_run: bool = False):
        ...
        self.temp_backup_path: Path | None = None
    ...
    def log_phase_end(
        self, phase_name: str, success: bool, details: dict[str, Any] = None
    ) -> None:
        ...
        if phase_name in self.results["phases"]:
            ...
            logger.info(
                f"{status_emoji} Phase Complete: {phase_name} ({phase['duration_seconds']:.1f}s)"
            )
    ...
    def run_command(
        self, command: list[str], timeout: int = 300, capture_output: bool = True
    ) -> tuple[bool, str, str]:
        ...
        try:
            if not self.temp_backup_path:
                self.log_phase_end(
                    "system_restore",
                    False,
                    {"error": "No backup file available for restore"},
                )
                return False
...
""",
    ...
    def run_full_drill(self) -> dict[str, Any]:
        ...
        successful_phases = len(
            [p for p in self.results["phases"].values() if p["status"] == "success"]
        )
        total_phases = len(self.results["phases"])
        self.results["metrics"]["success_rate"] = (
        ...
        if self.results["success"] and len(self.results["errors"]) == 0:
            logger.info(
                f"✅ DR Drill completed successfully in {self.results['total_duration_seconds']:.1f}s"
            )
        else:
            logger.error("❌ DR Drill completed with issues")
...
def print_drill_report(results: dict[str, Any]) -> None:
    ...
    print(
        f"🎯 **OVERALL STATUS: {status_emoji} {'PASSED' if results['success'] else 'FAILED'}**"
    )
    ...
    for phase_name, phase_data in results.get("phases", {}).items():
        ...
        print(
            f"   {status_emoji} {phase_name.replace('_', ' ').title()}: {phase_data['status']} ({phase_data['duration_seconds']:.1f}s)"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Run full DR drill (default)",
    )
    parser.add_argument(
