...
class GAChecker:
    ...
    def run_all_checks(self) -> GAAssessment:
        ...
        high_failures = len([r for r in self.results if r.status == 'FAIL' and r.category == 'HIGH'])
...
def print_results(assessment: GAAssessment, detailed: bool = False):
    ...
    critical_failures = [c for c in assessment.checks if c.status == 'FAIL' and c.category == 'CRITICAL']
    ...
    high_failures = [c for c in assessment.checks if c.status == 'FAIL' and c.category == 'HIGH']
    if high_failures:
...
def main():
    ...
    try:
        ...
        if args.json:
            ...
            # Minimal output
            print(f"{assessment.overall_status}: {assessment.passed}/{assessment.total_checks} checks passed")
