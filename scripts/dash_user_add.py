...
def main():
    ...
    parser.add_argument("--password", "-p", help="User password (will prompt if not provided)")
    parser.add_argument("--roles", "-r", default="viewer", help="Comma-separated roles (default: viewer)")
    parser.add_argument("--list", "-l", action="store_true", help="List all users")
    parser.add_argument("--stats", "-s", action="store_true", help="Show user statistics")
    parser.add_argument("--update-roles", "-u", help="Update roles for existing user")
    parser.add_argument("--deactivate", "-d", action="store_true", help="Deactivate user")
    parser.add_argument("--change-password", "-c", action="store_true", help="Change user password")
...
def create_user(user_store, email: str, password: str = None, roles_str: str = "viewer"):
    """Create a new user"""
...
def list_users(user_store):
    ...
    for user in users:
        ...
        if user["last_login_ts"]:
            last_login = datetime.fromtimestamp(user["last_login_ts"]).strftime("%Y-%m-%d %H:%M")
        else:
            last_login = "Never"
