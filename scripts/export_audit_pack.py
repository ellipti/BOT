...
class AuditExporter:
    ...
    def export_orders_csv(self, export_dir: Path, export_date: datetime) -> Path | None:
        """
        ...
        if not db_path:
            print("Warning: No order database found, skipping orders export")
            return None
    ...
    def export_fills_csv(self, export_dir: Path, export_date: datetime) -> Path | None:
        """
        ...
        if not db_path:
            print("Warning: No order database found, skipping fills export")
            return None
    ...
    def export_audit_logs(self, export_dir: Path, export_date: datetime) -> Path | None:
        """
        ...
        try:
            with open(audit_log_path, encoding="utf-8") as infile:
    ...
    def export_config_snapshot(self, export_dir: Path) -> Path | None:
        """
    ...
    def create_manifest(self, export_dir: Path, exported_files: list[Path]) -> Path:
        ...
        manifest_data = {
            ...
            "files": {},
            "summary": {"total_files": len(exported_files), "total_size": 0},
        }
    ...
    def export_daily_audit_pack(
        self, export_date: datetime | None = None, retention_days: int = 90
    ) -> Path:
        """
...
def main():
    ...
    parser.add_argument(
        "--date", help="Export date in YYYY-MM-DD format (defaults to yesterday)"
    )
    parser.add_argument(
        "--output-dir",
        default="artifacts",
