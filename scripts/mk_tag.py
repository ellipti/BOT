...
class ReleaseManager:
    ...
    def validate_version_format(self) -> bool:
        ...
        if not re.match(version_pattern, self.version):
            ...
            logger.error("Expected format: MAJOR.MINOR.PATCH[-prerelease][+buildmetadata]")
            return False
    ...
    def check_git_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.repo.is_dirty():
                status["clean"] = False
                status["uncommitted_changes"] = [item.a_path for item in self.repo.index.diff(None)]
...
"""
            ...
            if not self.dry_run:
                ...
                logger.info(f"DRY RUN: Would update CHANGELOG.md with {self.tag_name} entry")
        ...
        try:
            ...
            if pyproject_path.exists():
                ...
                if not self.dry_run:
                    ...
                    logger.info(f"DRY RUN: Would update pyproject.toml version to {self.version}")
    ...
    def generate_release_notes(self, custom_notes: Optional[str] = None) -> str:
...
"""
    ...
    def create_release_archive(self) -> Optional[Path]:
    ...
    def save_release_metadata(self, release_notes: str, archive_path: Optional[Path] = None) -> bool:
    ...
    def run_release_process(self, custom_notes: Optional[str] = None, push_tags: bool = False) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument('--push', action='store_true', help='Push tag to remote repository')
    parser.add_argument('--draft', action='store_true', help='Create draft release (dry run)')
