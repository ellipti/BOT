...
async def test_jwt_auth():
    ...
    # Test token creation
    token = create_access_token(user_id="admin@test.com", roles=["admin"])
    ...
    try:
        ...
        print(
            f"✅ Token verified: user={payload.get('sub')}, roles={payload.get('roles')}"
        )
    ...
    if user:
        print(
            f"✅ User authentication works: {user['email']} with roles {user['roles']}"
        )
    else:
