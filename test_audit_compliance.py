...
from audit.audit_logger import AuditLogger, get_audit_logger, audit_event, audit_order, audit_fill
...
class TestConfigSnapshotter:
    ...
    def test_snapshot_file_persistence(self, temp_dir, temp_config_files):
        ...
        snapshot_files = list(Path(f"{temp_dir}/snapshots").glob("config_snapshot_*.json"))
...
class TestAuditExporter:
    ...
    def sample_audit_log(self, temp_dir):
        ...
        sample_events = [
            {"ts": time.time(), "event": "OrderAccepted", "symbol": "EURUSD", "side": "BUY"},
            {"ts": time.time(), "event": "AlertSent", "alert_type": "price_alert", "message": "Price target reached"},
            {"ts": time.time(), "event": "Filled", "symbol": "EURUSD", "quantity": 0.1},
            {"ts": time.time(), "event": "ConfigChanged", "config_type": "risk_limit", "new_value": 0.01},
        ]
...
class TestAuditIntegration:
    ...
    def test_end_to_end_audit_flow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            audit_logger.write_order_event("OrderAccepted", "EURUSD", "BUY", 0.1, 1.1000)
