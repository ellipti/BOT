...
def run_pytest_suite(verbose: bool = False, security_only: bool = False) -> tuple[bool, str]:
...
def run_coverage_analysis(verbose: bool = False) -> tuple[bool, str]:
    ...
    try:
        ...
        report_result = subprocess.run(report_cmd, capture_output=True, text=True, cwd=".")
        
        # Generate HTML report
        html_cmd = [sys.executable, "-m", "coverage", "html", "-d", "htmlcov_redaction"]
        subprocess.run(html_cmd, capture_output=True, cwd=".")
...
'''
    ...
    try:
        result = subprocess.run([sys.executable, "-c", validation_code], 
                              capture_output=True, text=True, cwd=".")
...
def generate_test_report(results: dict, output_file: str = "log_redaction_test_report.md"):
...
def main():
    ...
    parser.add_argument('--report', '-r', default='log_redaction_test_report.md',
